...
describe('Product wish list for the buyers', () => {
    ...
    it('it should add product to a wishlist', async () => {
        ...
        response.body.should.have.property('wishedProductCreate');
        expect(response.body.message).to.equal('Product wished sucessfully!');
        response.body.wishedProductCreate.should.have.property('user_id');
        response.body.wishedProductCreate.should.have.property('product_id');
    });
...
describe('Wished products per product_id', () => {
    ...
    it('it should return 404 error code', async () => {
        const response = await chai
        .request(app)
        .get('/api/v1/926ade6c-21f7-4866-ae7f-360d1574839d/productWish')
        .set('Authorization', `Bearer ${ token }`)
        expect(response.status).to.equal(404)
    });
});    
