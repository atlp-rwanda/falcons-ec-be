...
export const checkout = async (req, res) => {
  try {
    ...
    if (cart.message !== 'ok') return res.status(400).json({ message: 'User cart not found' });
    ...
    for (const item of cart.existingItems) {
      ...
      if (!product) return res.status(400).json({ message: 'one of the cart items is not found' });
    ...
    const checkoutSession = await stripePayment('payment', customer, ['card'], line_items);
